...
| 6 | Generate different valid layout alternatives without moving locked objects. | `generate_route_alternatives` | Returned three distinct change signatures; checks confirmed locks, bounds, capacity, non-overlap, deterministic ordering, and bounded search. | Pass |
...
- Vitest: 3 files, 25 tests passing.
- Chromium: 7 mock-host browser workflows passing.
...
Rotated-polygon collision and jurisdiction-specific rules are outside the current scope.
